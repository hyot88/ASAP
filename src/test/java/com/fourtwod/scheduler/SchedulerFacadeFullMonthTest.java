...
@SpringBootTest
public class SchedulerFacadeFullMonthTest {
    ...
    public void before() {
        ...
        dayCount = YearMonth.from(lastLocalDate).lengthOfMonth();

        for (int day = 1; day <= dayCount; day++) {
            String strDate = lastMonthCondition + String.format("%02d", day);
            int missionType = 1;

            // 마지막 날은 3일짜리로 미션을 생성한다
            if (day == dayCount) {
                missionType = 3;
            }
            ...
            for (int i = 0; i < missionType; i++) {
                strDate = localDate.plusDays(i).format(DateTimeFormatter.BASIC_ISO_DATE);
    ...
    public void scheduler_풀미션성공보상() {
        ...
        // 변경된 티어, 티어 포인트 검증
        assertThat(user.getTier()).isEqualTo(3);
        ...
        // 히스토리 추가 여부 검증
        assertThat(missionHistoryDto.getMissionType()).startsWith("Huick").contains("30Day");
        ...
        assertThat(missionHistoryDto.getChangeTierPoint()).isEqualTo(300);
    }
}
